...
-- Provide the name for each region for every order, as well as the account name and the unit pric
...
-- Provide the name for each region for every order, as well as the account name and the unit pric
-- they paid (total_amt_usd/total) for the order. However, you should only provide the results if the
-- standard order quantity exceeds 100 and post order quantity exceeds 50. Your final table should have 3 columns: region name,
-- account name, and unit price. Sort the smallest unit price first
SELECT r.name region_name, a.name account_name, o.total_amt_usd / (o.total + 0.01) unit_price
FROM region r
JOIN sales_reps s
ON s.region_id = r.id
...
-- columns: account name and the different channels. You can try SELECT DISTINCT to narrow
...
SELECT o.occurred_at order_time, a.name account_name, o.total order_total, o.total_amt_usd total_amout
FROM accounts a 
JOIN orders o
ON o.account_id = a.id 
